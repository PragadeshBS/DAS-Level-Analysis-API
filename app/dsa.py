...
with open("app/deployables/lemmedWords.pkl", "rb") as f:
    lemmedWords = pickle.load(f)
antonyms = pd.read_pickle("app/deployables/antonyms.pkl")
with open("app/deployables/graph.pkl", "rb") as f:
    G = pickle.load(f)
flatDF_weight = pd.read_pickle("app/deployables/flatDF.pkl")
...
def ret_results(text):
        ...
        for token in note:
            ...
            if token.pos_ in ["VERB", "ADJ", "NOUN"]:
                ...
                if isNeg:
                    sims = np.array(
                        [
                            nlp2(outer(token.text.lower())).similarity(nlp2(lemmedWord))
                            for lemmedWord in lemmedWords
                        ]
                    )
                    ...
                    if len(word) == 0:
                        if (
                            len(
                                antonyms[antonyms[0] == lemmedWords[sims.argmax()]][
                                    1
                                ].values
                            )
                            == 0
                            ...
                            opword = antonyms[
                                antonyms[0] == lemmedWords[sims.argmax()]
                            ][1].values[0]
                            sims = np.array(
                                [
                                    nlp2(opword).similarity(nlp2(lemmedWord))
                                    for lemmedWord in lemmedWords
                                ]
                            )
                        ...
                        sims = np.array(
                            [
                                nlp2(word[0]).similarity(nlp2(lemmedWord))
                                for lemmedWord in lemmedWords
                            ]
                        )
                ...
                if outer(token.text.lower()) in lemmedWords:
                    vector_representation[
                        lemmedWords.index(outer(token.text.lower()))
                    ] = 0.99
                ...
                if outer(token.text.lower()) in G.nodes():
                    neibs = [
                        i
                        for i in nx.neighbors(G, outer(token.text.lower()))
                        if i in lemmedWords
                    ]
                    ...
                    sims = np.array(
                        [
                            nlp2(outer(token.text.lower())).similarity(nlp2(neib))
                            for neib in neibs
                        ]
                    )
                    if sims.max() < 0.5:
    ...
    if len(selectedWords) != 0:
        ...
        ] = pd.DataFrame(test.EmotionSum.to_list(), index=test.index)
        test = test.iloc[:, -7:]
        testSample = pd.concat(
            [
                pd.DataFrame(
                    vector_representation.reshape(1, -1), columns=flatDF_weight.columns
                ),
                test,
            ],
            axis=1,
